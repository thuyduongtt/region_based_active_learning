...
def conv_layer_renew(bottom, name, shape, training_state, strides=(1, 1), activation_func=tf.nn.relu, padding='same',
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        w_init = tf.compat.v1.truncated_normal_initializer(stddev=1)
        b_init = tf.compat.v1.constant_initializer(0.0)
        output = tf.cond(pred=training_state,
                         true_fn=lambda: tf.compat.v1.layers.conv2d(bottom, filters=shape[1], kernel_size=shape[0], strides=strides,
                                                  padding=padding,
                         ...
                         false_fn=lambda: tf.compat.v1.layers.conv2d(bottom, filters=shape[1], kernel_size=shape[0], strides=strides,
                                                  padding=padding,
                                                  dilation_rate=dilation_rate, activation=activation_func,
                                                  use_bias=bias_state, kernel_initializer=w_init,
...
def get_deconv_layer_weight(shape):
    ...
    init = tf.compat.v1.constant_initializer(value=weights, dtype=tf.float32)
    return init
...
def deconv_layer_renew(bottom, filter_shape, output_channel, name, strides, training_state, padding='same',
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        if bilinear_initialization is True:
            ...
            b_init = tf.compat.v1.constant_initializer(0.0)
        output = tf.cond(pred=training_state,
                         true_fn=lambda: tf.compat.v1.layers.conv2d_transpose(bottom, output_channel, filter_shape, strides=strides,
                                                            padding=padding, activation=tf.nn.relu, use_bias=True,
                                                            kernel_initializer=w_init, bias_initializer=b_init,
                                                            trainable=True, name=scope.name),
                         false_fn=lambda: tf.compat.v1.layers.conv2d_transpose(bottom, output_channel, filter_shape, strides=strides,
                                                            padding=padding, activation=tf.nn.relu, use_bias=True,
                                                            kernel_initializer=w_init, bias_initializer=b_init,
